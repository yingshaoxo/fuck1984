...
func dojob(jobtotal *int32, jobdone *int32, filetotal *int64, filedone *int64, input string, output string, doen bool,
	...
	for _, ss := range s {
		...
		if strings.HasPrefix(filepath.Base(ss), "fuck1984") {
			continue
		}
	...
	for _, ss := range s {
		...
		if strings.HasSuffix(filepath.Base(ss), "fuck1984") {
			if !doen {
				*jobtotal++
			}
		} else {
...
func defuck(workResultLock sync.WaitGroup, num *int32, key string, split int, ss string, flag bool,
	...
	outputss := strings.Replace(strings.TrimSuffix(ss, ".fuck1984"), input, output, -1)
...
func saveDone(output string, ss string) {
	...
	name := filepath.FromSlash(output + "/fuck1984DONE")
...
func fuck(workResultLock sync.WaitGroup, num *int32, key string, split int, ss string, flag bool,
	...
	// Open file for writing
	ofile, err := os.OpenFile(outputss+".fuck1984",
	...
	*filetotal = fi.Size()
	...
	for !fileend || !rb.Empty() {
		...
		for !rb.Empty() {
			...
			if cur > split {
				ofile.Close()
				bufferedWriter.Flush()
				ofile, err = os.OpenFile(outputss+".fuck1984"+"."+strconv.Itoa(post),
					os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0666)
				if err != nil {
					showError(err)
				}
	...
	fuckverify(key, outputss+".fuck1984", filetotal, filedone, md5str)
